...
def test_reports_page_loads(client, mock_mysql):
    ...
    mock_sessions = [
        {
            'id': 1, 'start_time': datetime(2025, 8, 20, 10, 0), 'end_time': datetime(2025, 8, 20, 11, 0),
            'total_minutes': 60, 'demands_worked': 'Demanda A', 'work_descriptions': 'Fiz X e Y'
        },
    ]
    ...
    assert b"Relat\xc3\xb3rio de Horas Trabalhadas" in response.data # Relatório de Horas Trabalhadas
...
def test_export_report(client, mock_mysql):
    ...
    mock_data = [
        {
            'start_time': datetime(2025, 8, 20, 10, 0), 'end_time': datetime(2025, 8, 20, 11, 0),
            'total_minutes': 60, 'demand_title': 'Demanda A', 'minutes_spent': 60, 'description': 'Descrição A'
        },
    ]
