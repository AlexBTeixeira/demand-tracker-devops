...
def test_save_new_demand(client, mock_mysql):
    ...
    assert response.status_code == 200
    assert (
        b"Priorize a Nova Demanda" in response.data
    )  # Deve redirecionar para a página de priorização
    ...
    mock_mysql.connection.cursor.return_value.execute.assert_any_call(
        """
            ...
            """,
        (
            "Nova Demanda de Teste",
            "Descricao da nova demanda.",
            "Em Fila",
            "10",
            6,
        ),  # max_priority + 1
...
def test_update_priorities(client, mock_mysql):
    ...
    """
    response = client.post(
        "/demands/update_priorities", json={"ordered_ids": ["3", "1", "2"]}
    )
    ...
    cursor_mock.execute.assert_any_call(
        "UPDATE demands SET priority = %s WHERE id = %s", (0, "3")
    )
    cursor_mock.execute.assert_any_call(
        "UPDATE demands SET priority = %s WHERE id = %s", (1, "1")
    )
    cursor_mock.execute.assert_any_call(
        "UPDATE demands SET priority = %s WHERE id = %s", (2, "2")
    )
